...
def x_rule_cleaner(self):
    ...
    for colour in list(self._rules.keys()):
        print(("\n * ", colour))
        lst_new_any = list()
        lst_new_all = list()
...
def x_stat_count_email(self, eml_in, bol_spam):
    ...
    if 'from' in eml_in:
        ...
        if not str_from in list(self._stat['senders'].keys()):
...
def x_get_statistics(self, key):
    ...
    if key in list(self._stat.keys()):
...
def x_show_pritty_statistics(self):
    ...
    print("   Senders stat:")
    lst_sndr = list()
    for sender in list(self._stat['senders'].keys()):
...
def x_report_to_global_stat_file(self, str_filename):
    ...
    # Merge the two dics
    for upd in list(dic_update.keys()):
        if not upd in list(dic_global.keys()):
    ...
    with open(str_filename, 'w') as fil_g:
        for sender in list(dic_global.keys()):
